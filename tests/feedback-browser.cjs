  ...
  try {
    ...
    for (const locale of ['ja', 'en']) {
      for (const viewport of [{width: 1280, height: 1000}, {width: 320, height: 740}]) {
        ...
        await open('success');
        assert.equal(await page.locator('.feedback-intro > p:not(.feedback-eyebrow)').count(), 2);
        for (const guidance of await page.locator('.feedback-intro > p:not(.feedback-eyebrow)').all()) assert.equal(await guidance.isVisible(), true);
        assert.equal(await page.locator('.feedback-eyebrow').isVisible(), true);
        ...
        for (const guidance of await page.locator('.feedback-intro > p:not(.feedback-eyebrow)').all()) assert.equal(await guidance.isVisible(), false);
        assert.equal(await page.locator('.feedback-intro h1').isVisible(), true);
        assert.equal(await page.locator('#feedback-success a').isVisible(), true);
        assert.equal(await page.locator('.feedback-eyebrow').isVisible(), true);
        assert.ok((await page.locator('#feedback-success').innerText()).includes(locale === 'ja' ? '個別の返信は行いません' : 'We do not send individual replies'));
        const introBox = await page.locator('.feedback-intro').boundingBox();
        const cardBox = await page.locator('.feedback-card').boundingBox();
        assert.ok(cardBox.y - introBox.y - introBox.height >= 0 && cardBox.y - introBox.y - introBox.height <= 60);
        assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
        await page.locator('.feedback-language a').last().focus();
        await page.keyboard.press('Tab');
        assert.equal(await page.locator('#feedback-success a').evaluate(el => el === document.activeElement), true);
